/* WOLFIE'S FIGHT — the half of the boss suite that starts once the warm-up hands over.
   Split out of pboss.js in v0.354a, which had grown to ten minutes and was starving whatever it
   shared the machine with. The arrival, the suspense, the warm-up, the X and the pose language
   stay there; everything here is about the fight itself: the stations, the escalation, whether it
   is dodgeable, the pound, the cooling beat, the bone speeds and the bird wall.
   The watchdog is installed identically in both halves — it is the audit that every k:"bone" was
   born at a live pentagram and none was born inside the cage, and it has to be watching whichever
   part of the fight the file is driving. Duplicated on purpose: each file has to be runnable on
   its own, which is the whole reason for the split. */
...
(async()=>{
  ...
  /* STRAIGHT TO THE FIGHT. The chain that gets here is pboss.js's subject, not this one's, so it
     is driven at speed: the intro clock is wound forward and the warm-up is told it is spent. */
  await pg.evaluate(()=>{ PK.active=true; pkBossStart(); });
  await pg.waitForTimeout(200);
  await pg.evaluate(()=>{ BOSS.introT=BOSS_INTRO-0.12; });
  ck(await waitFor(()=>BOSS.ph==="pawwarm", 20000), 'the slam never handed over to the warm-up');
  await pg.evaluate(()=>{ BOSS.paw.warmT=BOSS_WARM_DUR+1; BOSS.paw.xDone=true; });
  ck(await waitFor(()=>BOSS.ph!=="pawwarm", 20000), 'the warm-up never ended');
