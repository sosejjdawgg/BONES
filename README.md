...
- **Delivery Driver** — an isometric driving minigame: deliver parcels to the right houses along a street for cash, avoiding decoy (non-delivery) houses. The van accelerates the whole way down the road — there's a speedometer bottom-right, and the speed lines, rumble and engine note all build with it. Two separate, unambiguous ways to deliver: **swipe** (or tap THROW L/R) to fling the parcel now — fast, but a pure timing skill, since the path is exactly as wide as the doormat, so lining the van up with it is the whole read; overshoot and it clips the lawn, the window, or misses outright. Or **hold** — its own dedicated button — to pull the van over and walk the parcel to the door in person while a ticking countdown runs: a guaranteed doormat, but it costs several seconds and only pays the basic rate, and letting go too early, while the box is still in his hands, makes him trip and fumble it in front of the house. Pay rewards the fast, skillful option on purpose: a thrown doormat pays more than a held one, on the lawn but off the mat pays less again, a parcel never attempted −$7, and a broken window or a fumbled hand delivery −$42. Stringing accurate throws together builds a **streak bonus**, and however long the van is genuinely pinned at the top of the speedometer's red zone ticks up its own **top speed bonus** live on the dial — both get their own line in the post-run breakdown. Past the last house the controls become the brake: a **SLOW DOWN** sign, then a hatched bay you have to stop the van inside. Brake early and you stop short, brake late and you go into the wall. The whole run is rated against a 45-second par.
...
- **Bones economy** — every scared-off critter drops one bone. Spend bones between waves in the **shop** on stat upgrades (bark radius, cooldown, speed, max HP, knockback, healing, keen nose, agility) or rare **charms**. Or tap the bones counter any time to trade some for XP, money, or a treat.
- **The park opens up** — every wave cleared from wave 2 to wave 10 pulls the camera back one step (2.5%) and hands that same step of screen from the control pad to the view itself, so the park widens and the controls tighten as the run builds. It plays out inside the slow-motion final kill, with the mechanism ratcheting and settling as it lands. The progression finishes at the wave-10 clear and holds there for the rest of the run — the framing you fight wave 10 in is the framing you keep.
