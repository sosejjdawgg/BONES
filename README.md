...
- **Delivery Driver** — an isometric driving minigame: deliver parcels to the right houses along a street for cash, avoiding decoy (non-delivery) houses. The van accelerates the whole way down the road — there's a speedometer bottom-right, and the speed lines, rumble and engine note all build with it. Two separate, unambiguous ways to deliver: **swipe** (or tap THROW L/R) to fling the parcel now — fast, but a pure timing skill, since the path is exactly as wide as the doormat, so lining the van up with it is the whole read; overshoot and it clips the lawn, the window, or misses outright. Or **hold** — its own dedicated button — to pull the van over and walk the parcel to the door in person while a ticking countdown runs: a guaranteed doormat, but it costs several seconds, and letting go too early, while the box is still in his hands, makes him trip and fumble it in front of the house. Past the last house the controls become the brake: a **SLOW DOWN** sign, then a hatched bay you have to stop the van inside. Brake early and you stop short, brake late and you go into the wall. Pay is lopsided on purpose — a clean delivery is +$4, on the lawn but off the mat +$2, a parcel never attempted −$7, and a broken window or a fumbled hand delivery −$42 — and the run is rated against a 45-second par.
