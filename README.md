...
- **The park opens up** — from the wave-2 clear onward, every cleared wave pulls the camera back one step (2.5%, up to 50% out) and hands that same step of screen from the control pad to the view itself, so the park widens and the controls tighten the deeper a run goes. It plays out inside the slow-motion final kill, with the mechanism ratcheting and settling as it lands.
